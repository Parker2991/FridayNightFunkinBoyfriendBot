    ...
    switch (args[0]) {
      case 'version':
        context.source.sendFeedback({
          translate: '\n %s \n %s \n %s ',
            with: [
            { color: "green", text: `${buildstring}`},
              { color: "green", text: `${foundationbuildstring}`},
              { color: "green", text: 'BotEngine:Node-Minecraft-Protocol'},
          ]
        });
        ...
        context.source.sendFeedback({
          translate: '\n %s \n %s \n %s \n %s \n %s \n %s \n %s \n %s \n %s \n %s',
            ...
            { color: "green", text: `Hostname: ${os.hostname()}`},
              ...
              { color: "green", text: `Node js Version @${process.version}`},
               { color: "green", text: `Minecraft-Protocol Version @${MinecraftProtocolVersion}`},
              { color: "green", text: `Discord.js @${discordJSVersion}`},
      ...
      case 'logininfo':
        ...
        context.source.sendFeedback({
          translate: '\n %s',
            with: [
            { color: "green", text: `Bot Uptime: ${format(uptime)}`},
        ...
        context.source.sendFeedback({
            translate: '\n %s',
              with: [
              { color: "green", text: `§aMem §aused §a${Math.floor(os.freemem() / 1048576)} §aMiB §a/ §a${Math.floor(os.totalmem() / 1048576)} MiB`},
