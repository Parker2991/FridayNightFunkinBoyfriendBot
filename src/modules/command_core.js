async function command_core (bot, options) {
  ...
  bot.on('move', () => {
    ...
    if (bot.options.debug.core) {
       bot.tellraw(`Bots cords are x: ${bot.position.x} y: ${bot.position.y} z: ${bot.position.z}. this is for debugging! because bot.options.debug.enabled is set to true!`)      
    }
