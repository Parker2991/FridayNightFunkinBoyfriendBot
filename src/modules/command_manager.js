...
function command_manager(bot, options, config) {
  bot.commandManager = {
    commands: {},
    commandlist: [],
      ...
      try {
        if (source.sources.console && !source?.sources?.discord) {
          if (!command || !command.execute) {
            bot.console.warn({
              text: `Unknown Command ${commandName}. type "${bot.Console.prefix}help" for help`,
              color: 'dark_red'
            })
          }
        ...
        if (command?.trustLevel > 0) {
          if (command?.trustLevel === 1 && !source?.sources?.discord && !source?.sources?.console) {
            const hash = `${args[0]}`
            if (args.length === 0 && bot.hash && bot.owner && bot.hashing.hash) throw new CommandError([{
              text: 'Please provide an ',
              color: 'gray'
            }, {
              text: 'Trusted ',
              color: '#219696'
            ...
            if (hash !== bot.hash && hash !== bot.owner && hash !== bot.hashing.hash) throw new CommandError([{
              text: 'Invalid ',
              color: 'gray'
            }, {
              text: 'Trusted ',
            ...
            if (!hasRole) throw new CommandError({
              translate: 'You are not trusted!'
            })
          }
          bot?.hashing?.updateHash();
          if (args[0] === bot.hash) {
            bot.console.info({
              text: `Player ${player} UUID: ${uuid} Hash:${bot.hash}`,
              color: 'white'
            })
          } else if (args[0] === bot.owner) {
            bot.console.info({
              text: `Player ${player} UUID: ${uuid} Hash:${bot.owner}`,
              color: 'white'
            })
          }
          if (command?.trustLevel === 2 && !source?.sources?.discord && !source?.sources?.console) {
            const owner = `${args[0]}`
            if (args.length === 0 && bot.owner) throw new CommandError([{
              text: 'Please provide a ',
              color: 'red',
              color: 'gray'
            }, {
              text: 'Owner ',
            ...
            if (owner === bot.hash || owner === bot.hashing.hash) throw new CommandError([{
              text: "THATS A ",
              color: 'gray'
            }, {
              text: 'TRUSTED ',
            ...
            if (owner !== bot.owner) throw new CommandError([{
              text: "Invalid ",
              color: 'gray'
            }, {
              text: 'Owner ',
            ...
            if (!hasRole) throw new CommandError({
              translate: 'You are not the Owner!'
            })
          }
        ...
        if (!source.sources.discord && !source.sources.console) {
          if (bot.options.isCreayun || bot.options.isSavage) {
            if (error instanceof CommandError)
              bot.chat(bot.getMessageAsPrismarine(error._message)?.toMotd().replaceAll('§', '&'))
            else bot.chat(bot.getMessageAsPrismarine({
              translate: 'command.failed',
              color: 'dark_red'
            })?.toMotd(bot.registry.language).replaceAll('§', '&'))
          } else {
            if (error instanceof CommandError)
          ...
          const Embed = new EmbedBuilder()
            .setColor(`${config.Commands.colors.discord.error}`)
