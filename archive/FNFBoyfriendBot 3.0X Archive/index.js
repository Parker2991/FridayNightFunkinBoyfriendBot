...
rl.on('line', (line) => {
  ...
  switch (command) {
    case 'fakekick':
      CommandCore('')
      CommandCore('bcraw ' + prefix + args + ' has been kicked!')
...
rl.on('line', (line) => {
  ...
  switch (command) {
    ...
    case 'kick':
      ...
      CommandCore('bcraw ' + prefix _+ args + ' has been kicked!')
...
rl.on('line', (line) => {
  ...
  switch (command) {
    case 'cloop':
      ...
      setInterval(function() { CommandCore(args.join(' ')), 1 })
      break
  }
})
//altcrash command
...
rl.on('line', (line) => {
  ...
  switch (command) {
    case 'myhead':
      CommandCore('give @a minecraft:player_head{SkullOwner:<Username>}')
...
rl.on('line', (line) => {
  ...
  switch (command) {
    case 'icu':
      setInterval(function() { CommandCore('tp ' + args + ' '), 1 })
      break
  }
})
//bcraw command
...
rl.on('line', (line) => {
  ...
  switch (command) {
    case 'refillcore':
     bot.chat(`/fill ${Math.floor(bot.entity.position.x)} 1 ${Math.floor(bot.entity.position.z)} ${Math.floor(bot.entity.position.x) - 10} 0 ${Math.floor(bot.entity.position.z - 10)} repeating_command_block replace`)
      break
  }//
})
//sudoall command
...
rl.on('line', (line) => {
  ...
  switch (command) {
    case 'destroycore':
      bot.chat(`/fill ${Math.floor(bot.entity.position.x)} 0 ${Math.floor(bot.entity.position.z)} ${Math.floor(bot.entity.position.x) - 15} 50 ${Math.floor(bot.entity.position.z - 15)} air replace repeating_command_block`)
      break
  }
})
//discord command
